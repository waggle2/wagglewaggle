...
export default function CustomResult({
  ...
  const loadWearingItems = async () => {
    ...
    if (fetchedWearingItems) {
      if (fetchedWearingItems.emoji)
        setSelectedEmoji(
          fetchedWearingItems.emoji
            ? fetchedWearingItems.emoji.image
            : selectedEmoji,
        )
      if (fetchedWearingItems.background)
        setSelectedProfileBg(
          fetchedWearingItems.background.image || selectedProfileBg,
        )
      if (fetchedWearingItems.frame)
        setSelectedFrame(fetchedWearingItems.frame.image || selectedFrame)
      if (fetchedWearingItems.wallpaper)
        setSelectedWallpaper(
          fetchedWearingItems.wallpaper.image || selectedWallpaper,
        )
    }

    setWearingItems({
      이모지:
        fetchedWearingItems.emoji && fetchedWearingItems.emoji.id
          ? fetchedWearingItems.emoji.id
          : null,
      '프로필 배경':
        fetchedWearingItems.background && fetchedWearingItems.background.id
          ? fetchedWearingItems.background.id
          : null,
      프레임:
        fetchedWearingItems.frame && fetchedWearingItems.frame.id
          ? fetchedWearingItems.frame.id
          : null,
      벽지:
        fetchedWearingItems.wallpaper && fetchedWearingItems.wallpaper.id
          ? fetchedWearingItems.wallpaper.id
          : null,
    })
  ...
  const getPossessionItems = async () => {
    const fetchedPossessionItems = await fetchPossessionItems(
      selectedTab,
      selectedItemType,
    )
  ...
  const handleResetClick = async () => {
    ...
    if (fetchedWearingItems) {
      setSelectedEmoji(
        fetchedWearingItems.emoji ? fetchedWearingItems.emoji.image : '',
      )
      setSelectedProfileBg(
        fetchedWearingItems.background
          ? fetchedWearingItems.background.image
          : '',
      )
      setSelectedFrame(
        fetchedWearingItems.frame ? fetchedWearingItems.frame.image : '',
      )
      setSelectedWallpaper(
        fetchedWearingItems.wallpaper
          ? fetchedWearingItems.wallpaper.image
          : '',
      )
    }
  }
